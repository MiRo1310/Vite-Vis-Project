...
<template>
  <div v-for="(group, categoryName) in groupedRecipesByCategory" :key="categoryName" class="mb-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-4 3xl:grid-cols-6 gap-2">
      <div v-for="recipe in group" :key="recipe.id" @click="navigate(recipe.id)" class="border-2 p-2 rounded-md bg-card cursor-pointer">
        ...
        <div class="text-xs text-muted-foreground flex justify-between">
          ...
          <span v-if="recipe.preparationTimeMin"> {{ recipe.preparationTimeMin }} min </span>
          <span v-else>-</span>
        </div>
        <div class="text-xs text-muted-foreground flex justify-between">
          <span>Gesamtzeit: </span>
          <span v-if="recipe.totalTimeMin"> {{ recipe.totalTimeMin }} min </span>
          <span v-else>-</span>
        </div>
        <div class="text-xs text-muted-foreground flex justify-between">
