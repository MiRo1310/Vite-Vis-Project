    ...
    <CardContent class="grid grid-cols-2 gap-8">
      <div v-for="(event, index) in trashEvents" :key="index">
        <div class="flex justify-between ">
          <p class="text-accent-foreground/40 font-bold">{{ event.name }} </p>
          <Trash2
        ...
        <p class="text-xs text-accent-foreground/40 font-bold">{{ days[new Date(event.nextDate).getDay() as keyof Days]
          }}</p>
      </div>
    </CardContent>
  </Card>
