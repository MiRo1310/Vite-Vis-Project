  ...
  <div :class="cn('border-border/50 bg-background grid min-w-32 items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl', props.class)">
    <slot>
      ...
      <div class="grid gap-1.5">
        <div
          v-for="{ value, itemConfig, indicatorColor, key, date } in payload"
          :key="key"
          :class="
            cn(
        ...
        >
          ...
          <div :class="cn('flex flex-1 justify-between leading-none gap-2', nestLabel ? 'items-end' : 'items-center')">
            <div class="grid gap-1.5">
              <div v-if="nestLabel" class="font-medium">{{ tooltipLabel }}</div>
              <span class="text-muted-foreground"> {{ itemConfig?.label || value }} </span>
            </div>
            <span v-if="date">{{ new Date(date).toLocaleString() }}</span>
            <span class="text-foreground font-mono font-medium tabular-nums"> {{ value ? value.toLocaleString() : 0 }} </span>
          </div>
        </div>
      </div>
    </slot>
