...
<template>
  <ChartContainer :config="chartConfig" class="aspect-auto h-62.5 w-full" :cursor="false">
    <VisXYContainer :data="filterRange" :svg-defs="svgDefs" :margin="{ left: -40 }" :y-domain="[0, 1200]">
      <VisArea
        :x="(d: Data) => d.date"
        :y="[(d: Data) => d.value1, (d: Data) => d.value2]"
        :color="(_d: Data, i: number) => ['url(#fillMobile)', 'url(#fillDesktop)'][i]"
        :opacity="0.6"
      />
      <VisLine
        :x="(d: Data) => d.date"
        :y="[(d: Data) => d.value1, (d: Data) => d.value1 + d.value2]"
        :color="(_d: Data, i: number) => [chartConfig.value1.color, chartConfig.value2.color][i]"
        :line-width="1"
      />
      <VisAxis
        type="x"
      ...
      <ChartCrosshair
        ...
        "
        :color="(_d: Data, i: number) => [chartConfig.value1.color, chartConfig.value2.color][i % 2]"
      />
    </VisXYContainer>
    <ChartLegendContent />
  </ChartContainer>
