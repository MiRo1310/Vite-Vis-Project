...
<template>
    <div v-for="event in trashEvents" :key="event.name" class=" mt-2 flex items-end">
        <Trash2 :class="getColor(event.name)" class="h-8 w-8"></Trash2>
        <p class="border-2 rounded-full w-6 h-6 flex justify-center items-center ml-2">{{ event.daysLeft }}</p>
        <p class="ml-2">{{ transformDate(event.nextDate) }} </p>
    </div>
</template>
